...
| Recovery defeats intentional disconnect | Health timer, route service, or Admin action reconnects WARP after deliberate shutdown. | v0.4.1 recovery has explicit prerequisites but no intentional state yet. | Atomic root-owned `/run` intent; one root mutation lock shared by disconnect, recovery, repair, route-up, and reconnect; intent re-read under lock immediately before mutation; route-up refuses intent; corrupt/unsafe/inconsistent intent suppresses recovery; timers remain active; reboot/guarded CLI start may clear. | A root actor can delete the runtime record; reboot intentionally clears it and reconnects through normal boot. |
| Concurrent root mutations / intent TOCTOU | Disconnect, health recovery, repair, or route-up observes stale intent and interleaves an unsafe state transition. | Existing application serialization cannot cover timers, CLI, or systemd activity. | Stable root-owned `/run/warp-egress-gateway/mutation.lock`; exclusive lock held through mutation/postconditions; five-second bounded acquisition; intent and safety evidence re-read after lock; timeout is structured busy with no mutation. | Root code that ignores the frozen lock contract remains dangerous and must fail implementation review. |
...
| Audit-log injection or omission | Hides actions or misattributes an operator. | Journald provides host-controlled timestamps/metadata. | Structured fields; escaping and length bounds; application/helper events share request ID; helper names untrusted values `asserted_actor`, `asserted_role`, and `asserted_source_ip`; authoritative fields include effective root, verified sudo caller, fixed helper/protocol, and validated verb. Asserted metadata never affects authorization or command construction. | A compromised web process can lie about asserted identity; helper logs still establish the local service caller, not human cryptographic non-repudiation. |
...
- A healthcheck already running when disconnect begins cannot recover across
  the disconnect transaction.
- Disconnect already running when the health timer fires remains serialized.
- Routing repair racing health recovery permits exactly one locked mutation.
- Repeated route service starts while intent exists never reapply routes.
- The health timer stays active throughout intentional disconnect.
- The monitor stays active throughout intentional disconnect.
- Health returns `intentionally_disconnected` without failing its systemd unit.
- Lock timeout produces structured busy and never an unlocked mutation.
...
- Reboot-cleared `/run` intent restores the normal fail-closed boot path.
- Asserted audit metadata cannot alter authorization, the selected verb, or
