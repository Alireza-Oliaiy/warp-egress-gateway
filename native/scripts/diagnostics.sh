...
OUT=${1:-/tmp/warp-egress-diagnostics-$(date +%Y%m%d-%H%M%S).txt}
{
  ...
  echo "===== SERVICES ====="; systemctl status warp-gateway-firewall.service "wg-quick@${WARP_IF}.service" warp-gateway.service --no-pager || true
  ...
  echo "===== RECENT LOGS ====="; journalctl -u warp-gateway-firewall.service -u "wg-quick@${WARP_IF}.service" -u warp-gateway.service -u warp-gateway-healthcheck.service -n 200 --no-pager || true
} >"${OUT}"
chmod 600 "${OUT}"
printf '%s\n' "${OUT}"
